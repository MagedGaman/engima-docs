...
This guide covers the most common issues users encounter when working with WebRun, along with detailed solutions and preventive measures.
...
3. **Block Unnecessary Domains with Policies**

   Attach an [automation policy](/concepts/policies) with domain rules to block ad networks and analytics domains that waste tokens.
...
4. **Avoid Sites Requiring Auth**
   ...
   Use an [automation policy](/concepts/policies) with domain rules to block login-required domains, or provide credentials upfront with [secrets](/usage-guides/secrets).
...
3. **Avoid CAPTCHA-Heavy Sites**
   ...
   Use an [automation policy](/concepts/policies) with domain rules to block known CAPTCHA-heavy domains.
