...
description: "Troubleshooting guide for frequently encountered issues with WebRun"
icon: "bug"
---
   ...
   async function waitForSessionActive(sessionId, apiKey, maxWait = 30000) {
     ...
     while (Date.now() - startTime < maxWait) {
       const response = await fetch(
         `https://api.webrun.ai/start/session-status/${sessionId}`,
         {
...
2. **Terminate and Recreate**
   ...
   // Create new session
   const newSession = await fetch("https://api.webrun.ai/start/start-session", {
     method: "POST",
     headers: {
       "Content-Type": "application/json",
       "Authorization": `Bearer ${apiKey}`
...
3. **Check System Status**
   - Visit [status.webrun.ai](https://status.webrun.ai)
...
2. **Check WebSocket Events**
   ...
   const ws = new WebSocket(`wss://api.webrun.ai/ws?sessionId=${sessionId}`);
...
1. **Set Token Limits**
   ```javascript
   const response = await fetch("https://api.webrun.ai/start/start-session", {
     method: "POST",
     headers: {
       "Content-Type": "application/json",
       "Authorization": `Bearer ${apiKey}`
...
1. **Verify Session is Active**
   ...
   async function getStreamUrl(sessionId, apiKey) {
     ...
     // Get stream URL
     return `https://api.webrun.ai/stream/${sessionId}`;
   }
   ```
...
2. **Check Network Configuration**
   ...
   // Test connection
   const streamUrl = `https://api.webrun.ai/stream/${sessionId}`;
...
3. **Use Alternative Streaming Methods**
   ...
   const eventSource = new EventSource(
     `https://api.webrun.ai/stream/sse/${sessionId}?apiKey=${apiKey}`
   );
   ...
   function connectWebSocket(sessionId, apiKey, maxRetries = 3) {
     ...
     function connect() {
       const ws = new WebSocket(
         `wss://api.webrun.ai/ws?sessionId=${sessionId}&apiKey=${apiKey}`
       );
...
<CardGroup cols={2}>
  ...
  <Card title="Get Support" icon="life-ring" href="mailto:support@webrun.ai">
