...
This is the User-Agent the *rendering browser* sends to sites for compatibility. Separately, WebRun's automated traffic is identifiable and verifiable at the network layer through the `WebRunAgent` bot identity - used to allow-list WebRun on Cloudflare-protected sites and other bot-management systems. See [Bot Identification](/concepts/bot-identification).
...
- [API Reference](/api-reference/endpoints): All endpoints with examples
