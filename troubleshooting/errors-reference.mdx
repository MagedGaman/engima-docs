...
description: "Complete reference of all WebRun API error codes, status codes, and solutions"
...
| [INSUFFICIENT_BALANCE](#insufficient_balance) | Add credits at webrun.ai/billing | [Link](#insufficient_balance) |
...
- [API Keys Management](https://app.webrun.ai)
...
**Related:**
- [Regenerating API Keys](https://app.webrun.ai)
...
**Related:**
- [API Limits](/api-reference/rate-limits)
...
- [Usage Tracking](https://app.webrun.ai/usage)
...
async function executeTaskWithRetry(prompt, apiKey) {
  ...
  try {
    ...
    const session = await makeRequestWithRetry(
      "https://api.webrun.ai/start/start-session",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
