...
<Tip>
For workflows that span multiple sessions (e.g., daily recurring tasks that need to stay logged in), use [environments](/environments/overview) to persist cookies, files, and browser data between sessions.
</Tip>
...
curl -X POST https://api.webrun.ai/start/send-message \
  ...
  -d '{
    ...
    "message": {
      "actionType": "newTask",
      "newState": "start",
      "prompt": "Now add the first item to cart"
    }
  }'
```
  ...
  const task1 = await fetch(`${BASE}/start/send-message`, {
    ...
    body: JSON.stringify({
      ...
      message: {
        actionType: "newTask",
        newState: "start",
        prompt: "Search for 'wireless keyboard'"
      }
    })
  }).then(r => r.json());
  ...
  const task2 = await fetch(`${BASE}/start/send-message`, {
    ...
    body: JSON.stringify({
      ...
      message: {
        actionType: "newTask",
        newState: "start",
        prompt: "Add the first result to cart"
      }
    })
  }).then(r => r.json());
...
curl -X POST https://api.webrun.ai/start/run-task \
  ...
  -d '{
    "task": {
      "prompt": "Search Google for Anthropic and return the first result"
    }
  }'
```
...
| `environmentId` | string | null | [Environment](/environments/overview) ID to attach |
| `task` | object | null | Initial task configuration (see below) |

**task parameters:**
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `prompt` | string | "" | Initial task description (can be empty for sessions without initial task) |
| `startingUrl` | string | null | Starting URL (e.g., `"https://amazon.com"`) |
| `maxDuration` | number | 300000 | Max task duration in ms (default and max: 5 minutes) |
...
| `outputType` | string | "text" | Output format: `"text"`, `"structured_json"`, or `"structured_csv"` |
| `outputSchema` | object | null | JSON Schema for structured output |
| `files` | string[] | [] | File IDs to attach to the task ([details](/environments/file-uploads)) |
