...
<Tip>
**Prefer secrets for login credentials.** If you know which sites the agent will authenticate with, use the [`secrets` parameter](/usage-guides/secrets) to provide credentials upfront. This avoids guardrail interruptions entirely and enables fully automated workflows. Secrets are never stored — they are discarded when the session ends.
</Tip>
