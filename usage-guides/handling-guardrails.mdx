...
Chat routing is enabled by default. To turn it off for a specific session, set `allowConciergeReachOut` to `false` at session creation:
...
The flag is set at session creation and applies for the lifetime of the session. There is no way to change it later — start a new session if you need different behavior.
