...
Attach an [automation policy](/concepts/policies) with domain rules to block unnecessary sites and prevent wasted navigation. Policies can also restrict action types, flag keywords, and detect sensitive data.
...
```javascript
{
  ...
  "initialTask": {
    "taskDetails": "Download all product images from the catalog"
  }
}
```
...
- [ ] Use [automation policies](/concepts/policies) to prevent accidental navigation
