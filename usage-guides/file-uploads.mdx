...
2. **Attach fileIds** to a task so the agent can use them
...
Once you have `fileId` values, include them in the request body when starting a task. All three task endpoints accept file references:
...
Include `fileIds` in the request body alongside the `initialTask`:
...
Include `fileIds` in the `message` object when sending a `newTask` action:
...
| `POST /start/start-session` | `fileIds` | Request body (top-level) |
| `POST /start/run-task` | `fileIds` | Request body |
| `POST /start/send-message` | `fileIds` | `message` object (for `newTask` actions) |
