...
description: "Upload, download, and manage the browser profile stored in an environment"
icon: "code"
---

Manage the browser profile stored within an environment. Profiles store persistent Chrome browser data — cookies, local storage, extensions — that is reused across sessions.
...
Returns all environments (and their profile data) for the authenticated user, sorted by newest first.
...
Creates a new environment with an empty browser profile. You can populate it by [uploading data](#upload-profile-data) or by running a session with the environment attached.
...
Permanently deletes an environment and its stored browser data.
...
Returns a temporary signed URL to download the environment's browser data as a `.tar.gz` archive. The URL expires after 1 hour.
...
Uploads browser profile data as a `.tar.gz` archive to an environment. Replaces any existing profile data.
...
<Accordion title="Related">
  <CardGroup cols={2}>
    <Card title="Profiles Overview" icon="user-gear" href="/environments/profiles">
      Understanding browser profiles within environments
    </Card>
