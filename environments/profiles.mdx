...
| **[Sync from local Chrome](/environments/sync-profiles)** | You're already logged into sites locally and want to transfer that state |
...
You can download an environment's browser profile as a `.tar.gz` archive for backup or transfer. The download endpoint returns a signed URL that expires after 1 hour:

```bash
curl https://connect.webrun.ai/profiles/<ENV_ID>/download \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
{
  "success": true,
  "downloadUrl": "https://storage.example.com/profiles/<ENV_ID>.tar.gz?token=..."
}
```

See the [Profile Management API](/environments/profile-management-api#download-profile-data) for full details.
...
<CardGroup cols={3}>
  ...
  <Card title="Profile Management API" icon="book" href="/environments/profile-management-api">
    Download, upload, and manage profile data
  </Card>
</CardGroup>
