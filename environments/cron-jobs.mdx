...
Cron jobs let you schedule browser automation tasks that execute automatically on a recurring basis. Each cron job is tied to an [environment](/environments/overview), so it inherits the browser profile, stored cookies, and persistent files — no re-authentication required between runs.

---

## Why Cron Jobs

| Without cron jobs | With cron jobs |
|-------------------|----------------|
...
| Webhook delivery requires custom wiring per task | Delivery mode (webhook, bot, or both) is configured once per job |

---

## How It Works

1. **Create** a cron job linked to an environment, with a schedule and task details.
...
```
Create job → System runs on schedule → Results delivered → Run history logged
```
...
Every cron job requires a `scheduleType` that determines when and how often it runs.
...
| `paused` | Job is paused and will not execute until resumed |

A job is automatically paused after reaching `maxConsecutiveFailures` (default: 5). Resume it with the [resume endpoint](#resume-a-cron-job) after resolving the issue.
...
Returns all cron jobs for the authenticated user, sorted by creation date (newest first).
...
Returns a single cron job with its 10 most recent runs.
...
Update any field on an existing cron job. Only include the fields you want to change.
...
Permanently deletes a cron job and all its run history.
...
Pause an active job. The job will stop executing until resumed.
...
Resume a paused job. Resets the consecutive failure counter and recomputes the next run time.
...
Immediately trigger a job run outside of its normal schedule. The run executes in the background.
...
<Note>
Manual triggers run independently of the job's schedule. The job's `nextRunAt` is not affected.
</Note>
...
Returns paginated execution history for a cron job.
