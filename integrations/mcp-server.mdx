...
The WebRun MCP (Model Context Protocol) server allows AI assistants like Claude Desktop and Cline to access browser automation capabilities directly through their native tool interfaces.

It exposes six tools (task execution, session management, status checks, session control, and manual takeover) with built-in session lifecycle handling and streaming for long-running tasks.
...
```json
{
  "mcpServers": {
    "webrun": {
      "url": "https://api.webrun.ai/mcp/YOUR_API_KEY"
    }
  }
}
```

To connect to a specific environment, append the environment ID to your API key. An environment is a complete persistent remote machine. It maintains its own Chrome browser, file manager, and desktop state across sessions. This means browser profiles, downloaded files, cookies, and any other local state are preserved between tasks, allowing you to pick up exactly where you left off.

```json
{
  "mcpServers": {
    "webrun": {
      "url": "https://api.webrun.ai/mcp/YOUR_API_KEY:ENVIRONMENT_ID"
    }
  }
}
```
...
<Tip>
Without an environment ID, each MCP session starts with a fresh machine. Use environments when you need persistent state, such as staying logged into websites, keeping downloaded files, or maintaining browser bookmarks and extensions.
</Tip>
...
| `maxDuration` | number | No | 300000 | Max task duration in milliseconds |
| `reach_out_mode` | string | No | `"guardrail_only"` | Controls proactive chat messages to a user (Telegram/WhatsApp/Slack/Discord/Teams) connected to the same environment. One of `"off"`, `"guardrail_only"`, or `"full"`. Use `"off"` for sessions handling private data |
...
| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `prompt` | string | No | "" | Initial task (can be empty to just create session) |
| `startingUrl` | string | No | null | Starting URL |
| `maxDuration` | number | No | 300000 | Max task duration in ms |
| `reach_out_mode` | string | No | `"guardrail_only"` | Controls proactive chat messages to a user (Telegram/WhatsApp/Slack/Discord/Teams) connected to the same environment. One of `"off"`, `"guardrail_only"`, or `"full"`. Locked at session creation |
...
1. Uses create_session:
   {
     "prompt": "Go to example.com and attempt to log in",
...
1. Uses browser_task:
   {
     "prompt": "Go to example.com and extract all links"
   }
...
2. Claude analyzes error and retries with adjusted parameters:
   Uses browser_task:
   {
     "prompt": "Go to example.com, wait for page to fully load, then extract all links",
     "maxDuration": 120000
   }
   → Succeeds
```
...
2. For each company (1-10):
   ...
   {
     "sessionId": "abc123",
     "prompt": "Search for [Company Name] and extract: founded date, CEO, main products, revenue"
   }
...
3. **Ensure API key format:**
   - API key should start with `wr_`
   - No extra spaces or quotes
   - Example: `"url": "https://api.webrun.ai/mcp/wr_xxxxxxxxxxxx"`
...
1. **Verify API key is active:**
   - Go to [webrun.ai/settings](https://webrun.ai/settings)
...
3. **Test API key via curl:**
   ...
   curl -X POST https://api.webrun.ai/start/run-task \
     -H "Authorization: Bearer wr_xxxxxxxxxxxx" \
...
**Cause:** WebRun sessions expire after 5 minutes of inactivity (fixed, not configurable).
...
|------|---------------------|---------------------|
| `browser_task` | `prompt` | `startingUrl`, `maxDuration`, `reach_out_mode` |
| `create_session` | - | `prompt`, `startingUrl`, `maxDuration`, `reach_out_mode` |
| `send_task_to_session` | `sessionId`, `prompt` | `terminateOnCompletion` |
...
- Examples:
  - `60000` = 1 minute max task duration
  - `120000` = 2 minutes max task duration
  - `300000` = 5 minutes max task duration (default)
...
- One of:
  - `"off"`: No proactive messages. Guardrails and results stay on the MCP channel only. Useful for sessions handling private data.
  - `"guardrail_only"`: Bot pings the chat user only on guardrails (CAPTCHA, 2FA, verification, login, etc.).
  - `"full"`: Bot pings on guardrails **and** delivers the task result to chat when the task completes.
- Default: `"guardrail_only"`
- Available on `browser_task` and `create_session` only; locked at session creation
- See [Routing Guardrails to a Chat User](/usage-guides/handling-guardrails#routing-guardrails-to-a-chat-user) for the full flow
...
**User:** "Use WebRun to search for 'Claude AI features' on Google and tell me what you find."

**Claude's Response:**
```
I'll search Google for Claude AI features using the browser automation tool.

[Uses browser_task with prompt: "Go to google.com, search for 'Claude AI features', and summarize the top 3 results"]
...
[Uses send_task_to_session with prompt: "Search for wireless keyboards under $50 and extract the top 3 results with names, prices, and ratings"]
...
[Uses send_task_to_session with prompt: "Extract the account balance from the dashboard"]
