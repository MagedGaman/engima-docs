...
| `allow_concierge_reach_out` | boolean | No | true | When `false`, guardrails from this session are not relayed to chat users (Telegram/WhatsApp/Slack/Discord/Teams) connected to the same environment. Use `false` for sessions handling private data |
...
| `allow_concierge_reach_out` | boolean | No | true | When `false`, guardrails from this session are not relayed to chat users (Telegram/WhatsApp/Slack/Discord/Teams) connected to the same environment. Locked at session creation |
...
3. **Ensure API key format:**
   ...
   - Example: `"url": "https://connect.webrun.ai/mcp/enig_xxxxxxxxxxxx"`
...
2. **Check URL format:**
   ...
   // ✅ Correct
   "url": "https://connect.webrun.ai/mcp/enig_xxxxxxxxxxxx"
   ```
...
|------|---------------------|---------------------|
| `browser_task` | `prompt` | `startingUrl`, `maxDuration`, `allow_concierge_reach_out` |
| `create_session` | - | `prompt`, `startingUrl`, `maxDuration`, `allow_concierge_reach_out` |
...
**allow_concierge_reach_out** (boolean)
- `true`: Guardrails raised by the session can be forwarded to a chat user (Telegram, WhatsApp, Slack, Discord, Teams) connected to the same environment when no one is watching the session live
- `false`: Guardrails are never forwarded to chat users — useful for sessions handling private data
- Default: `true`
