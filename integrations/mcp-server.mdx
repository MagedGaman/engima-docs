...
| `Environment-Id` | No | The 24-character hex ID of a persistent environment. Omit it and every task gets a fresh disposable machine |
...
<Tip>
Without an environment, each task starts on a fresh disposable machine. Use environments when you need persistent state, such as staying logged into websites, keeping downloaded files, or maintaining browser bookmarks and extensions.
</Tip>
...
| `environmentId` | string | No | connection default | Environment to run in. Without one, a disposable instance is used |
...
- 24-character hex ID of a persistent environment
- Overrides the connection's `Environment-Id` header for this one call
- Omit it on a connection with no environment to get a disposable machine
