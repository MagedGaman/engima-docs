...
| `allow_concierge_reach_out` | boolean | No | When `false`, guardrails from the underlying session are not relayed to chat users (Telegram/WhatsApp/Slack/Discord/Teams) connected to the same environment. Default: `true`. See [Routing Guardrails to a Chat User](/usage-guides/handling-guardrails#routing-guardrails-to-a-chat-user) |
