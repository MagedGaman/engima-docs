...
description: "Use WebRun with OpenAI SDK, LangChain, LlamaIndex, and Vercel AI SDK"
icon: "robot"
---
...
const client = new OpenAI({
  baseURL: 'https://api.webrun.ai/v1',
  apiKey: 'YOUR_API_KEY' // Your WebRun API key
});

const completion = await client.chat.completions.create({
  model: 'webrun-browser-1',
  messages: [
...
const model = openai('webrun-browser-1', {
  baseURL: 'https://api.webrun.ai/v1',
  apiKey: 'YOUR_API_KEY'
});
...
export async function POST(req: Request) {
  ...
  const model = openai('webrun-browser-1', {
    baseURL: 'https://api.webrun.ai/v1',
    apiKey: process.env.WEBRUN_API_KEY
  });
...
```bash
POST https://api.webrun.ai/v1/chat/completions
...
|-----------|------|----------|-------------|
| `model` | string | Yes | Must be `"webrun-browser-1"` |
| `messages` | array | Yes | Array of message objects with `role` and `content` |
| `stream` | boolean | No | Enable streaming responses (default: false) |
| `max_tokens` | number | No | Maximum tokens in response (default: 2000) |
| `reach_out_mode` | string | No | Controls proactive chat messages to a user (Telegram/WhatsApp/Slack/Discord/Teams) connected to the underlying session's environment. One of `"off"` (no messages), `"guardrail_only"` (only on guardrails), or `"full"` (guardrails plus task results). Default: `"guardrail_only"`. See [Routing Guardrails to a Chat User](/usage-guides/handling-guardrails#routing-guardrails-to-a-chat-user) |
...
```
data: {"id":"chatcmpl-a1b2c3","object":"chat.completion.chunk","created":1704067200,"model":"webrun-browser-1","choices":[{"index":0,"delta":{"role":"assistant","content":"I"},"finish_reason":null}]}

data: {"id":"chatcmpl-a1b2c3","object":"chat.completion.chunk","created":1704067200,"model":"webrun-browser-1","choices":[{"index":0,"delta":{"content":" found"},"finish_reason":null}]}

data: {"id":"chatcmpl-a1b2c3","object":"chat.completion.chunk","created":1704067200,"model":"webrun-browser-1","choices":[{"index":0,"delta":{"content":" 5"},"finish_reason":null}]}

...

data: {"id":"chatcmpl-a1b2c3","object":"chat.completion.chunk","created":1704067200,"model":"webrun-browser-1","choices":[{"index":0,"delta":{},"finish_reason":"stop"}]}
...
|------------|-------------|
| `webrun-browser-1` | Default browser automation model (Claude Sonnet 3.5) |
...
const session = await fetch('https://api.webrun.ai/start/start-session', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
    'Authorization': 'Bearer YOUR_API_KEY'
  },
  body: JSON.stringify({ task: { prompt: 'Go to amazon.com' } })
}).then(r => r.json());

await fetch('https://api.webrun.ai/start/send-message', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
    'Authorization': 'Bearer YOUR_API_KEY'
  ...
  body: JSON.stringify({
    ...
    message: {
      actionType: 'newTask',
      newState: 'start',
      prompt: 'Search for keyboards' // Same session, still on Amazon
    }
  })
});
```
...
async function researchTopic(topic) {
  ...
  const cost = completion.webrun?.cost || 0;
