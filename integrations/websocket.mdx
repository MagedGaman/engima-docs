...
socket.on("message", (data) => {
  if (data.type === "task_completed") {
    ...
    if (data.data.files?.length) {
      console.log("Files:", data.data.files.map(f => f.downloadUrl));
    }
    if (data.data.network?.length) {
      console.log("Network requests:", data.data.network.length);
    }
    socket.disconnect();
  }
});
...
socket.on("instance:disconnected", (data) => {
  console.warn("Instance disconnected");
...
async function runTaskWithWebSocket(prompt, apiKey) {
    ...
    socket.on("message", (data) => {
      switch (data.type) {
        ...
        case "guardrail_trigger":
          console.error("Guardrail:", data.data.value);
          socket.disconnect();
...
class WebRunWebSocketClient {
  ...
  async createSession(prompt) {
    ...
    this.socket = io("https://api.webrun.ai", {
  ...
  onGuardrail(data) {
    console.warn("Guardrail:", data.value);
  ...
  sendTask(prompt, terminateOnCompletion = false) {
...
// Usage
const client = new WebRunWebSocketClient("wr_xxx");
