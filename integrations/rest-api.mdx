...
### Example 3: Using a Browser Profile

Attach a [browser profile](/profiles/overview) to reuse saved cookies and login sessions across tasks:

```javascript
const API_KEY = "enig_xxxxxxxxxxxx";

async function runWithProfile(profileId, taskDetails) {
  const response = await fetch("https://connect.webrun.ai/start/run-task", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
// First run: log in and save cookies to profile
await runWithProfile("683a1f2e4b0c1d2e3f4a5b6c", "Go to amazon.com and log in");

// Subsequent runs: profile has saved cookies, no login needed
const result = await runWithProfile(
  "683a1f2e4b0c1d2e3f4a5b6c",
...
### Example 4: Authenticated Task with Secrets
...
async function loginAndExtractWithSecrets(url) {
  ...
  const session = await fetch(`${BASE}/start/start-session`, {
    ...
    body: JSON.stringify({
      mode: "default",
      initialTask: {
        taskDetails: `Go to ${url}, log in, and extract all user data from the dashboard`,
        startingPoint: url,
        terminateOnCompletion: true,
        secrets: [
          {
...
<Accordion title="Related Guides">
  <CardGroup cols={2}>
    ...
    <Card title="Browser Profiles" icon="user-gear" href="/profiles/overview">
      Persist cookies and browser data across sessions
    </Card>
    <Card title="File Uploads" icon="file-arrow-up" href="/usage-guides/file-uploads">
