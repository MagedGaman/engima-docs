...
### Example 3: Using an Environment

Attach an [environment](/environments/overview) to reuse saved cookies, login sessions, and stored files across tasks:

```javascript
const API_KEY = "enig_xxxxxxxxxxxx";

async function runWithEnvironment(environmentId, taskDetails) {
  const response = await fetch("https://connect.webrun.ai/start/run-task", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
// First run: log in and save cookies to environment
await runWithEnvironment("683a1f2e4b0c1d2e3f4a5b6c", "Go to amazon.com and log in");
...
<Accordion title="Related Guides">
  <CardGroup cols={2}>
    ...
    <Card title="Environments" icon="box" href="/environments/overview">
      Persist browser profile and files across sessions
    </Card>
