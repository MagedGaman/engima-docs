...
```json
{
  "success": true,
  "sessionId": "a1b2c3d4e5f6",
  "socketURL": "https://api.webrun.ai",
  "streaming": {
    "webRTCURL": "https://a1b2c3d4e5f6.webrun.ai/whep",
    "webViewURL": "https://a1b2c3d4e5f6.webrun.ai",
...
```bash
curl -X POST https://api.webrun.ai/start/send-message \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -d '{
    "sessionId": "a1b2c3d4e5f6",
...
async function runTask(prompt, apiKey) {
  const response = await fetch("https://api.webrun.ai/start/run-task", {
    method: "POST",
    headers: {
...
async function pollForResult(sessionId, taskId, apiKey) {
  ...
  for (let i = 0; i < maxAttempts; i++) {
    const res = await fetch(
      `https://api.webrun.ai/task/${sessionId}/${taskId}`,
...
// Usage
const result = await runTask("Search Google for Anthropic", "wr_xxx");
...
async function searchGoogle(query) {
  const response = await fetch("https://api.webrun.ai/start/run-task", {
    ...
    body: JSON.stringify({
      task: {
        prompt: `Search Google for "${query}" and summarize the first result`
      }
    })
  });
...
```javascript
const API_KEY = "wr_xxxxxxxxxxxx";
const BASE = "https://api.webrun.ai";
  ...
  const searchTask = await fetch(`${BASE}/start/send-message`, {
    ...
    body: JSON.stringify({
      ...
      message: {
        actionType: "newTask",
        newState: "start",
        prompt: `Search for "${productName}" and list the top 3 results with prices`
      }
    })
  }).then(r => r.json());
...
// WebRun-managed proxy (specify country)
const response = await fetch("https://api.webrun.ai/start/run-task", {
  method: "POST",
  headers: {
    "Content-Type": "application/json",
...
See the [Proxies guide](/usage-guides/proxies) for custom proxy configuration and best practices.

---

### Example 4: Using an Environment

Attach an [environment](/environments/overview) to reuse saved cookies and login sessions across tasks:

```javascript
const API_KEY = "wr_xxxxxxxxxxxx";

async function runWithEnvironment(environmentId, prompt) {
  const response = await fetch("https://api.webrun.ai/start/run-task", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
// First run: log in and save cookies to the environment's profile
await runWithEnvironment("<ENV_ID>", "Go to amazon.com and log in");

// Subsequent runs: environment has saved cookies, no login needed
const result = await runWithEnvironment(
  "<ENV_ID>",
  "Go to amazon.com and check my recent orders"
);
console.log(result);
```
...
Use [secrets](/usage-guides/secrets) to provide credentials upfront so the agent can authenticate without guardrail interruptions:

```javascript
async function loginAndExtractWithSecrets(url) {
  const BASE = "https://api.webrun.ai";
  const API_KEY = "wr_xxxxxxxxxxxx";
  ...
  const session = await fetch(`${BASE}/start/start-session`, {
    ...
    body: JSON.stringify({
      mode: "default",
      task: {
        prompt: `Go to ${url}, log in, and extract all user data from the dashboard`,
        startingUrl: url,
        terminateOnCompletion: true,
        secrets: [
          {
            match: `*.${new URL(url).hostname}`,
            fields: {
  ...
  // Poll for result, no guardrail handling needed
  const taskId = session.taskId || session.sessionId;
  return await pollForResult(session.sessionId, taskId, API_KEY);
}
```
    ...
    await fetch(`${BASE}/start/send-message`, {
      ...
      body: JSON.stringify({
        ...
        message: {
          ...
          prompt: `Username: ${credentials.username}, Password: ${credentials.password}`,
          newState: "resume"
        }
      })
    });
  ...
  const extractTask = await fetch(`${BASE}/start/send-message`, {
    ...
    body: JSON.stringify({
      ...
      message: {
        actionType: "newTask",
        newState: "start",
        prompt: "Extract all user data from the dashboard",
        terminateOnCompletion: true
      }
    })
  }).then(r => r.json());
...
```javascript
async function executeTaskWithRetry(prompt, apiKey, maxRetries = 3) {
