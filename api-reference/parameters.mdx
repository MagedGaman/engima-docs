...
- **Required:** No
- **Default:** `null`
- **Description:** ID of an [environment](/environments/overview) to attach to the session. When set, the browser loads the environment's saved profile data (cookies, local storage, extensions) before starting the task, and the session has access to all files stored in that environment.
...
<Tip>
Use environments to skip login steps. Log in once with an environment attached, then reuse that environment for future sessions — the saved cookies keep you authenticated.
</Tip>
...
See [File Uploads guide](/environments/file-uploads) for the full upload workflow including persistent and orphan files.
