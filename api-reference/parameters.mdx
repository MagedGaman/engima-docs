...
<Note>
`policyId` is a top-level session parameter, alongside `environmentId` and `proxy`. A session can have a policy, an environment, both, or neither. Policies are a superset of `avoidDomains` — see [Automation Policies](/concepts/policies) for details.
</Note>
...
- **Required:** No
- **Default:** `[]`
- **Description:** List of domains the agent should avoid navigating to. Useful for preventing unwanted redirects or blocking third-party sites. For more advanced control over domains, URLs, keywords, action types, and sensitive data, use [automation policies](/concepts/policies).
