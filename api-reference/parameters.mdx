...
- **Required:** No
- **Default:** `300000` (5 minutes of inactivity)
- **Description:** Inactivity timeout before automatic termination. Sessions can run indefinitely while active. The timer resets each time activity occurs.
...
- **Required:** No
- **Default:** `"text"`
- **Values:** `"text"`, `"structured"`
- **Description:** Specifies the output format. Use `"structured"` with `outputSchema` to get validated JSON output.
...
<Note>
When using `outputType: "structured"`, you must also provide an `outputSchema` to define the expected response structure.
</Note>
...
- **Required:** No (required when `outputType` is `"structured"`)
- **Description:** JSON Schema defining the expected response structure. The AI agent's output will be validated against this schema.
...
<Tip>
Use `"additionalProperties": false` to ensure the output contains only the fields you specify.
</Tip>
...
All [Initial Task Parameters](#initial-task-parameters) are available, plus:
...
```typescript
type OutputType = "text" | "structured";
```
...
interface WebhookConfig {
  ...
  url: string;
  auth?: string;
  submittedData: "full_response" | "structured_output" | "just_ping";
}
```
...
|-----------|-----|-----|-------|
| `maxDuration` | 0 | unlimited | Milliseconds (inactivity timeout) |
