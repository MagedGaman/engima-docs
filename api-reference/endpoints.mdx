...
<Note>
The `initialTask` object contains all task-specific parameters. Use `startingPoint` to specify the URL where the task should begin. Use `environmentId` (top-level) to attach an [environment](/environments/overview) with saved browser profile and persistent files.
</Note>
...
See [File Uploads guide](/environments/file-uploads) for complete examples including persistent and orphan files.
...
| `/files/upload` | POST | Required | 10/min per user | Upload files (persistent or orphan) |
| `/start/health` | GET | Not required | None | System health check |
| `/environments` | GET | Required | 100/15min per IP | List all environments |
| `/environments` | POST | Required | 100/15min per IP | Create an environment |
| `/environments/:id` | DELETE | Required | 100/15min per IP | Delete an environment |
| `/environments/:id/upload` | POST | Required | 100/15min per IP | Upload browser profile to environment |
| `/environments/:id/files` | GET | Required | 100/15min per IP | List files in environment |
| `/environments/:id/files/:fileId` | DELETE | Required | 100/15min per IP | Delete a file from environment |
| `/profiles/:id/download` | GET | Required | 100/15min per IP | Download environment profile data |
