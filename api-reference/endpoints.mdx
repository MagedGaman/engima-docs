...
Use the returned `fileId` values in `/start/start-session` (top-level `fileIds`), `/start/run-task` (`files`), or `/start/send-message` (`message.fileIds` for `newTask` actions).
