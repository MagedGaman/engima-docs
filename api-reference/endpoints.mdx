...
<Note>
The `initialTask` object contains all task-specific parameters. Use `startingPoint` to specify the URL where the task should begin. Use `environmentId` (top-level) to attach an [environment](/environments/overview) with saved cookies, browser state, and persistent files. Use `proxy` (top-level) to route the session through a [proxy server](/usage-guides/proxies).
</Note>
...
| `/cron-jobs` | GET | Required | — | List all cron jobs |
| `/cron-jobs` | POST | Required | — | Create a cron job |
| `/cron-jobs/:id` | GET | Required | — | Get a cron job with recent runs |
| `/cron-jobs/:id` | PUT | Required | — | Update a cron job |
| `/cron-jobs/:id` | DELETE | Required | — | Delete a cron job |
| `/cron-jobs/:id/pause` | POST | Required | — | Pause a cron job |
| `/cron-jobs/:id/resume` | POST | Required | — | Resume a paused cron job |
| `/cron-jobs/:id/trigger` | POST | Required | — | Manually trigger a cron job |
| `/cron-jobs/:id/runs` | GET | Required | — | List cron job run history |
