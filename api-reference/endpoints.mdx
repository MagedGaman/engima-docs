...
<Note>
The `initialTask` object contains all task-specific parameters. Use `startingPoint` to specify the URL where the task should begin. Use `environmentId` (top-level) to attach an [environment](/environments/overview) with saved cookies, browser state, and persistent files.
</Note>
