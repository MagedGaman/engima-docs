...
```json
{
  ...
  "pending": true,
  "usage": {
    "cost": 0.0067  // Increases as task continues
  }
}
```
...
| `maxDuration` | number | 300000 | Session timeout in ms (max: 300000 = 5 min) |
...
- **Session ID not found?** Sessions timeout after `maxDuration` (default: 5 minutes). Check if the session expired.
