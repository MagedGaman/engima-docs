...
| `task` | object | null | Task configuration |

**Task-level (inside `task`):**
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `prompt` | string | `""` | Task description in natural language |
| `startingUrl` | string | null | Starting URL (skips navigation) |
...
| `files` | string[] | `[]` | File IDs to attach ([details](/environments/file-uploads)) |
...
|--------|---------|--------------|
| 400 | Bad request | Missing required field (usually `task.prompt`) |
...
- **Session ID not found?** Sessions expire after 5 minutes of inactivity.
...
const response = await fetch("https://connect.webrun.ai/start/run-task", {
  ...
  body: JSON.stringify({
    task: { prompt: "Search Google for Anthropic" }
  })
});
