...
fetch('https://api.webrun.ai/start/run-task', {
...
const result = await makeRequestWithRetry(
  'https://api.webrun.ai/start/run-task',
  {
    method: 'POST',
    headers: {
    ...
    body: JSON.stringify({ task: { prompt: 'Search Google for Anthropic' } })
  }
);
```
...
class RateLimitedClient {
  ...
  async request(endpoint, options = {}) {
    ...
    const response = await fetch(`https://api.webrun.ai${endpoint}`, {
      ...options,
      headers: {
        ...options.headers,
...
const result = await client.request('/start/run-task', {
  ...
  body: JSON.stringify({ task: { prompt: 'Search Google' } })
});
```
...
for (let i = 0; i < 10; i++) {
  await fetch('https://api.webrun.ai/start/run-task', {
    method: 'POST',
    headers: { 'Authorization': 'Bearer YOUR_API_KEY', 'Content-Type': 'application/json' },
    body: JSON.stringify({ task: { prompt: `Task ${i}` } })
  });
}
...
const session = await fetch('https://api.webrun.ai/start/start-session', {
  method: 'POST',
  headers: { 'Authorization': 'Bearer YOUR_API_KEY', 'Content-Type': 'application/json' },
  body: JSON.stringify({ task: { prompt: 'Initial task' } })
}).then(r => r.json());
...
for (let i = 0; i < 10; i++) {
  await fetch('https://api.webrun.ai/start/send-message', {
    ...
    body: JSON.stringify({
      ...
      message: { actionType: 'newTask', newState: 'start', prompt: `Task ${i}` }
    })
  });
}
...
while (true) {
  const result = await fetch('https://api.webrun.ai/start/run-task', {
    method: 'POST',
    headers: { 'Authorization': 'Bearer YOUR_API_KEY', 'Content-Type': 'application/json' },
    body: JSON.stringify({ task: { prompt: 'Search Google' } })
  }).then(r => r.json());
...
const task = await fetch('https://api.webrun.ai/start/run-task', {
  method: 'POST',
  headers: { 'Authorization': 'Bearer YOUR_API_KEY', 'Content-Type': 'application/json' },
  body: JSON.stringify({ task: { prompt: 'Search Google' } })
}).then(r => r.json());
...
setInterval(async () => {
  const status = await fetch('https://api.webrun.ai/start/send-message', {
    method: 'POST',
    headers: { 'Authorization': 'Bearer YOUR_API_KEY', 'Content-Type': 'application/json' },
    body: JSON.stringify({
      sessionId: 'abc123',
...
const socket = io('https://api.webrun.ai', {
