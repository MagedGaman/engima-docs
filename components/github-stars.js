// ClickHouse GitHub Stars Injector
console.log('ClickHouse GitHub stars script loaded');

(function() {
  'use strict';

  const githubLinkHTML = `
    <a class="clickhouse-github-link"
       style="display: none; align-items: center; gap: 0.5rem; font-size: 0.875rem; font-weight: 500;"
       target="_blank"
  ...
  function injectGithubStars() {
    ...
    const navbarLinks = document.querySelectorAll('#fern-header a');

    let githubLink = null;
    navbarLinks.forEach(link => {
      if (link.href && link.href.includes('github.com/ClickHouse/ClickHouse')) {
        githubLink = link;
      }
    });
    ...
    // Check if already replaced and still in DOM
    if (injectedLink && document.body.contains(injectedLink)) {
      return true;
    }

    console.log('Injecting GitHub stars link');

    // Create a temporary container
    const temp = document.createElement('div');
    temp.innerHTML = githubLinkHTML;
    const newLink = temp.firstElementChild;

    // Replace the old link with the new one
    githubLink.parentNode.replaceChild(newLink, githubLink);
    injectedLink = newLink;

    console.log('GitHub stars link injected');
    return true;
  }

  // Continuously check and re-inject if needed
  function checkAndInject() {
    const header = document.getElementById('fern-header');
    if (header) {
      const githubLinks = document.querySelectorAll('#fern-header a[href*="github.com/ClickHouse"]');

      // Check if our custom link is gone or if there's a Fern default link
      let needsInjection = false;

      if (!injectedLink || !document.body.contains(injectedLink)) {
        needsInjection = githubLinks.length > 0;
      } else {
        // Check if Fern re-added the original link
        githubLinks.forEach(link => {
          if (!link.classList.contains('clickhouse-github-link')) {
            needsInjection = true;
          }
        });
      }
