...
- Files: Deletes cached schema files stored locally in the `format_schema_path`, generated when `format_schema_source` is set to `query`. Note: If no target is specified, both caches are cleared.
