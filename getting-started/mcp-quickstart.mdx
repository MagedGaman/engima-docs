...
- **Everything with a config file** (Claude Code, Cursor, Claude Desktop, VS Code, Cline) uses the same URL plus two headers. Start at [Connect with an API key](#connect-with-an-api-key).
...
An environment is a persistent remote machine with its own browser, file system, and desktop, so logins, cookies, and downloaded files survive between tasks. Without one, each task gets a fresh disposable machine.
