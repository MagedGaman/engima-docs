...
| **Zapier** | 30s (fixed, cannot change) | Requires polling | Always use polling pattern |
...
- Method: POST
- URL: `https://api.webrun.ai/start/run-task`
...
### Important: Increase HTTP Timeout
...
- URL: Construct from Step 2:
  - `https://api.webrun.ai/task/{{steps.2.sessionId}}/{{steps.2.taskId}}`
...
| Loop/retry support | Native | Native | Limited |
