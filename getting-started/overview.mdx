...
Download files from one platform and upload them to another—moving documents, reports, and data between systems autonomously.

**Example task:** "Download the monthly sales report from our analytics dashboard and upload it to the shared Google Drive folder."
...
For login credentials, you can bypass guardrails entirely by providing [secrets](/usage-guides/secrets) upfront when creating the session. Secrets are matched to websites by domain pattern and are never stored — they exist only in the session's memory and are discarded when the session ends.

Alternatively, use [environments](/environments/overview) to persist login cookies and files across sessions — log in once and reuse the saved browser state for future tasks.

[Guardrails documentation](/concepts/guardrails) | [Secrets documentation](/usage-guides/secrets) | [Environments documentation](/environments/overview)
