...
<Tip>
`profileId` is optional. Pass it to reuse saved cookies, logins, and browser state from a [profile](/profiles/overview). Omit it to start with a clean browser.
</Tip>
...
```javascript
async function runTask(taskDetails, apiKey, profileId = null) {
  // Start the task
  const response = await fetch("https://connect.webrun.ai/start/run-task", {
    ...
    body: JSON.stringify({ taskDetails, profileId })
  });
