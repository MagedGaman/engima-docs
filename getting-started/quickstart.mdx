...
<Warning>
**Sessions cost money until terminated.** Sessions remain billable until they time out (5 min max) or you explicitly terminate them. Always call `terminate` when done, or set `terminateOnCompletion: true` on your last task.
</Warning>
...
<Tip>
**Poll every 2-3 seconds.** Faster polling wastes API quota; slower polling adds unnecessary latency. Most tasks complete within 10-40 seconds.
</Tip>
