...
- **policyId**: Attach an [automation policy](/concepts/policies) to enforce runtime rules on agent actions. Policies cover domain restrictions, keyword filters, action types, URL patterns, and sensitive data — a superset of `avoidDomains`. This is a top-level parameter, not nested inside `initialTask`.
- **avoidDomains**: Prevents accidental navigation to unwanted domains. For more advanced control, use [policies](/concepts/policies).
...
- **environmentId**: Attach an [environment](/environments/overview) to reuse saved cookies, local storage, files, and other browser data. This is a top-level parameter, not nested inside `initialTask`.
