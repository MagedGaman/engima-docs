...
**Orphaned** → Instance disconnected unexpectedly (5-minute reconnection window)

Orphaned sessions are rare. If a browser instance loses connection, the session waits 5 minutes for reconnection. If it doesn't reconnect, the session fails.
...
```javascript
{
  ...
  "initialTask": {
    ...
    "maxDuration": 300000,                        // Timeout in ms (max: 300000 = 5 min)
    ...
    "outputType": "structured",                   // "text" or "structured"
...
- **maxDuration**: Enforced server-side. Sessions cannot exceed 5 minutes.
...
**5-minute maximum:**
Sessions automatically terminate after 5 minutes. Any running task stops, and you're billed only for actual compute time used.
