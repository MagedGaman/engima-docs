...
```javascript
{
  ...
  "allowConciergeReachOut": true,                    // Optional: relay guardrails to a chat user
...
- **allowConciergeReachOut**: When `true` (default), guardrails raised by the session can be forwarded to a chat user (Telegram, WhatsApp, Slack, Discord, Teams) connected to the same [environment](/environments/overview) — but only when no client is actively watching the session. Set to `false` for sessions handling private data you don't want surfaced through chat. See [Handling Guardrails](/usage-guides/handling-guardrails#routing-guardrails-to-a-chat-user).
