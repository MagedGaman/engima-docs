...
- **policyId**: Attach an [automation policy](/concepts/policies) to enforce runtime rules on agent actions. Policies cover domain restrictions, keyword filters, action types, URL patterns, and sensitive data detection. This is a top-level parameter, not nested inside `initialTask`.
