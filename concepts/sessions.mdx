...
```javascript
{
  "environmentId": "683a1f2e4b0c1d2e3f4a5b6c",   // Optional: attach environment (browser profile + files)
...
- **environmentId**: Attach an [environment](/environments/overview) to load its saved browser profile and access its stored files. This is a top-level parameter, not nested inside `initialTask`.
...
**Complete isolation:**
No shared cookies, cache, or session state between sessions. Each session starts with a clean browser — unless an [environment](/environments/overview) is attached. Environments persist the browser profile (cookies, local storage) and files across sessions.
