...
- **profileId**: Attach a [browser profile](/profiles/overview) to reuse saved cookies, local storage, and other browser data. The profile ID is a top-level parameter, not nested inside `initialTask`.
...
**Complete isolation:**
No shared cookies, cache, or session state between sessions. Each session starts with a clean browser — unless a [profile](/profiles/overview) is attached. Profiles let you persist cookies, local storage, and other browser data across sessions.
