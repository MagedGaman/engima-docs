...
| `fileIds` / `files` | string[] | `[]` | File IDs to attach for upload tasks ([details](/usage-guides/file-uploads)) |
| `profileId` | string | null | Browser profile ID to attach to the session ([details](/profiles/overview)). This is a top-level parameter, not nested inside the task. |
...
<CardGroup cols={2}>
  ...
  <Card title="Browser Profiles" icon="user-gear" href="/profiles/overview">
    Reuse cookies and browser state across tasks
  </Card>
  <Card title="File Uploads" icon="file-arrow-up" href="/usage-guides/file-uploads">
    Attach files to tasks for the agent to upload
  </Card>
</CardGroup>
